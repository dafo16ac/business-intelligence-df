...
if __name__ == '__main__':
    ...
    def update_table_blocks(radioItems_clusters_value, start_date, end_date):
        ...
        try:
            children_nation_rev = children_callback.groupby('Country')['Revenues by day'].mean().sort_values(ascending=False).index[
                0]
        except IndexError:
        ...
        try:
            children_nation_rating = children_callback.groupby('Country')['Rating'].mean().sort_values(ascending=False).index[0]
        except IndexError:
            children_nation_rating = 'null'

        try:
            adults_32_nation_rating = adults_32_callback.groupby('Country')['Rating'].mean().sort_values(ascending=False).index[0]
        except IndexError:
            adults_32_nation_rating = 'null'

        try:
            adults_48_nation_rating = adults_48_callback.groupby('Country')['Rating'].mean().sort_values(ascending=False).index[0]
        except IndexError:
            adults_48_nation_rating = 'null'

        try:
            adults_66_nation_rating = adults_66_callback.groupby('Country')['Rating'].mean().sort_values(ascending=False).index[0]
        except IndexError:
            adults_66_nation_rating = 'null'

        try:
            adults_plus_nation_rating = adults_plus_callback.groupby('Country')['Rating'].mean().sort_values(ascending=False).index[
                0]
        except IndexError:
            adults_plus_nation_rating = 'null'

        try:
            children_nation_upselling = children_callback.groupby('Country')['Upselling'].mean().sort_values(ascending=False).index[
                0]
        except IndexError:
        ...
        table_a = pd.DataFrame(data=[
            [children_mean_callback, adults_32_mean_callback, adults_48_mean_callback, adults_66_mean_callback, adults_plus_mean_callback],
            [childrenRev_mean_callback, youngRev_32_mean_callback, adultsRev_48_mean_callback, adultsRev_66_mean_callback, adultsRev_plus_mean_callback],
            [children_rating_callback, adults_32_rating_callback, adults_48_rating_callback, adults_66_rating_callback, adults_plus_rating_callback],
            [children_nights_callback, adults_32_nights_callback, adults_48_nights_callback, adults_66_nights_callback, adults_plus_nights_callback],
            [children_wd_callback, adults_32_wd_callback, adults_48_wd_callback, adults_66_wd_callback, adults_plus_wd_callback],
            [children_we_callback, adults_32_we_callback, adults_48_we_callback, adults_66_we_callback, adults_plus_we_callback],
